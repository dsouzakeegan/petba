...
export class AuthServiceService {
  ...
  img() {
    ...
    // return (this.imageUrl = `${imgUrl}Api/`);
  }
  img2() {
...
//  async doPostHttp(url:string ,params:any){
      ...
      .then((res) => {
        ...
        try {
          ...
          if (typeof res.data === 'string' && (res.data.startsWith('{') || res.data.startsWith('['))) {
            ...
            console.warn("Parsed JSON Result:", result);
            resolve(result);
          } else {
          ...
          console.warn("Server Raw Response:", res.data); // Log raw response to inspect
